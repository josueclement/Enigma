...
public class TripleDesSicTests
{
    ...
    public async Task CsvEncryptTest(byte[] key, byte[] iv, byte[] data, byte[] encrypted)
    {
        ...
        var service = new BlockCipherServiceFactory().CreateSicBlockCipherService(engineFactory.CreateTripleDesEngine);
    ...
    public static IEnumerable<object[]> GetCsvValues()
    {
        ...
        return File.ReadAllLines(@"BlockCiphers\tripledes-ctr.csv")
            .Skip(1)
            .Select(line =>
            {
